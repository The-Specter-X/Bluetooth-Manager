...
app identity, missing-tray recovery, hotplug/restart and single-instance exit.
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="mytooth-smoke-") as temp:
        ...
        try:
            ...
            app = launch([binary, "--background", "--debug"], "mytooth.log", {"WAYLAND_DEBUG": "client"})
            ...
            wait_for(lambda: 'set_app_id("io.github.the_specter_x.Mytooth")' in
                     (logs / "mytooth.log").read_text(), "visible native Wayland window after missing tray host")
            ...
            print("PASS: native Wayland identity, missing-tray recovery, hotplug, service restart, single instance and quit")
        finally:
